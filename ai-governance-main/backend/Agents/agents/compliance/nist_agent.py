"""
NIST AI RMF Compliance Sub-Agent
----------------------------------
Computes the NIST AI RMF framework score, recommendations, and detailed
analysis from already-scored questions (per_question_scores) and the
control matrix. This module owns ONLY NIST-specific scoring logic.
"""
...
def calculate_score(
    ...
    """Calculate the NIST AI RMF maturity score (0-100) from question and
    control evidence weighted specifically for the NIST framework."""
    total = 0.0
    max_total = 0.0
...
def recommend_next_steps(score: float, controls: Dict[str, Any]) -> List[str]:
    """Generate NIST AI RMF specific recommendations."""
    recs: List[str] = []

    if score < 70:
        recs.append(
            "Operationalize the NIST AI RMF: define metrics for bias, "
            "robustness, and incident response."
        )
...
def generate_detailed_analysis(
    ...
    """Generate NIST AI RMF specific contributing/missing analysis."""
